...
func runServe(cmd *cobra.Command, args []string) {
	...
	// Database engine auto-tuning. Runs BEFORE the engine starts, because the
...
// runEngineAutoTuning sizes a database engine to the container it was given and
// writes the result where the engine reads it.
//
...
func runEngineAutoTuning(engineName, wakeName string) error {
	...
	calc, err := autotune.NewEngineCalculator(engine, wake, logger)
	if err != nil {
		return err
	}
	...
	// Nothing to write when there was no limit to size against; the engine keeps
	// its own defaults and the warning has already been logged.
	if len(cfg.Settings) == 0 {
		return nil
	}

	path := engineConfigPath(engine)
	if override := os.Getenv("CBOX_ENGINE_CONFIG_PATH"); override != "" {
		path = override
	}
	...
	header := fmt.Sprintf("# Generated by cbox-init for a %dMB container (%s).\n"+
		"# Edit the engine's own config to override; this file is rewritten on start.\n",
		cfg.MemoryLimitMB, cfg.WakeMode)

	if err := os.WriteFile(path, []byte(header+cfg.RenderConfig()), 0o644); err != nil {
		return fmt.Errorf("failed to write %s: %w", path, err)
	}

	for key, value := range cfg.ToEnvVars() {
		if err := os.Setenv(key, value); err != nil {
			slog.Warn("Failed to set engine env var", "key", key, "error", err)
		}
	}
...
// engineConfigPath is where each engine picks a fragment up without being told.
func engineConfigPath(engine autotune.Engine) string {
	switch engine {
	case autotune.EnginePercona:
		// Included by my.cnf's !includedir; the zz- prefix makes it load last so
		// it wins over the image's own defaults.
		return "/etc/my.cnf.d/zz-cbox-autotune.cnf"
	default:
		// Valkey has no include directory, so the image points the server at
		// this file directly.
		return "/etc/valkey/cbox-autotune.conf"
	}
}
